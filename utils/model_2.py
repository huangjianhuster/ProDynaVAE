...
def Hidden_layer_neurons(latent_dim, original_dim, number_of_hidden_layers):
    ...
    # Distribute neurons evenly among hidden layers
    for i in range(1, number_of_hidden_layers + 1):
        neuron_layer[i] = min_nodes + i * ((max_nodes - min_nodes) // (number_of_hidden_layers + 1))
...
def sampling(args):
    ...
    epsilon = tf.keras.backend.random_normal(shape=(tf.keras.backend.shape(z_mean)[0], tf.keras.backend.shape(z_mean)[1]), mean=0., stddev=1.0)
...
def vae_encoder(original_dim, latent_dim=2, num_of_hidden_layer=4):
    ...
    #This Lambda layer for mean and variance

    # create encoder
    encoder = tf.keras.models.Model(encoder_input, [z_mean, z_log_var, z])
    print('"""""""""""""""""""', encoder)
    return encoder, z_mean, z_log_var,z, encoder_input
...
def build_vae(original_dim, latent_dim=2, num_of_hidden_layer=4, rate=0.0001):
    ...
    encoder, z_mean, z_log_var,z, encoder_input = vae_encoder(
    ...
    z_decoded = decoder(z) #(encoder(encoder_input)[2])
